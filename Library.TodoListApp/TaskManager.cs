...
namespace Library.TodoListApp
{
    public static class TaskManager
    {
        public const int MAX_PAGE_SIZE = 3;
        public static List<TodoItem> Items { get; set; } = default;

        public static string CreateTask(string name, string description, DateTime deadline, bool isCompleted)
        {
            var item = new TodoTask(){ Name=name, Description = description, Deadline = deadline, IsCompleted = isCompleted };
            Items.Add(item);
            return item.Id;
        }
        public static string CreateAppointment(string name, string description, DateTime start, DateTime stop, List<string> attendees)
        {
            var item = new TodoAppointment(){ Name=name, Description = description, Start = start, Stop = stop, Attendees = attendees };
            Items.Add(item);
            return item.Id;
        }
        ...
        public static bool EditTask(string id, string newName, string newDesc, DateTime newStart = default, DateTime newStop = default, string newAttendees = default, DateTime newDeadline = default)
        {
            ...
            if (found is TodoTask t && newDeadline != default) t.Deadline = newDeadline;
            if (found is TodoAppointment a)
            {
        ...
        {
            var task = FindItem(id) as TodoTask;
            if (task == default(TodoTask)) return false;
            task.IsCompleted = true;
            return true;
        }
        public static void ListOutstandingTasks()
        {
            bool FindAllOutstanding(TodoItem item)
            {
                if (item is TodoTask t) return !t.IsCompleted;
                return false;
            }
        ...
        public static void NavigateTaskList<T>(List<T> items)
        {
            ...
            while (isNavigating)
            {
                navigator.GetCurrentPage().Values.ToList().ForEach(item => (item as TodoItem)?.Log());
        ...
        public static TodoItem FindItem(string id) => Items.Find(task => task.Id == id);
    }
}